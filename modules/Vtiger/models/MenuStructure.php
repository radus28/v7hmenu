...
class Vtiger_MenuStructure_Model extends Vtiger_Base_Model
{
    ...
    public function allMenuSelected()
    {
        $selectedModules = array();
        $sql = "SELECT tabid FROM vtiger_allmenu order by `position`";
        $adb = \PearDatabase::getInstance();
        $result = $adb->pquery($sql, array());
        if($result){
        for ($i = 0; $i <= $adb->num_rows($result); $i++) {
            $tabid = $adb->query_result($result,$i);
            $moduleModel = Vtiger_Module_Model::getInstance($tabid);
            if($moduleModel){
                $moduleName = getTabname($tabid);
                $selectedModules[$moduleName] = $moduleModel;
            }
        }
        }
